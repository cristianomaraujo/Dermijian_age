...
Original file is located at
    https://colab.research.google.com/drive/1OhUaOLVAoiwyETYCqqsrNOsoA60mibUD
...
colunas = ['sex','Demirjian 37', 'Demirjian 36', 'Demirjian 35', 'Demirjian 34',
       'Demirjian 33', 'Demirjian 32', 'Demirjian 31', 'IDADE CRONOLÓGICA']
...
file_path2 = '/content/dataset.xlsx'
df2 = pd.read_excel(file_path2)
colunas2 = ['IDADE DENTÁRIA','IDADE CRONOLÓGICA', 'Demirjian 36']

df2 = df2.loc[:, colunas2]

coluna_grupo2 = df2.pop('IDADE CRONOLÓGICA')
df2.insert(0, 'IDADE CRONOLÓGICA', coluna_grupo2)
...
df2 = df2.drop(columns=['Demirjian 36'])

#Dataset split
X2 = df2.iloc[:,1:]
y2 = df2.iloc[:,0]
X_train2, X_test2, y_train2, y_test2 = train_test_split(X2,y2,test_size = 0.2,random_state=RANDOM_STATE, shuffle=True)
...
from math import sqrt
import numpy as np
mse_dental_age = mean_squared_error(y_test, X_test2['IDADE DENTÁRIA'])
mae_dental_age = mean_absolute_error(y_test, X_test2['IDADE DENTÁRIA'])
rmse_dental_age = sqrt(mse_dental_age)
r2_dental_age = r2_score(y_test, X_test2['IDADE DENTÁRIA'])

print(f"MSE Dental Age: {mse_dental_age}")
print(f"RMSE Dental Age: {rmse_dental_age}")
print(f"MAE Dental Age: {mae_dental_age}")
print(f"R2 Dental Age: {r2_dental_age}")

# Garantir consistência no filtro de outliers
q1 = df2["IDADE DENTÁRIA"].quantile(0.25)
q3 = df2["IDADE DENTÁRIA"].quantile(0.75)
iqr = q3 - q1
lower_bound = q1 - 1.5 * iqr
upper_bound = q3 + 1.5 * iqr

# Filtrar os dados
df_filtered = df2[(df2["IDADE DENTÁRIA"] >= lower_bound) & (df2["IDADE DENTÁRIA"] <= upper_bound)]

# Dados para métricas
y_true = df_filtered["IDADE CRONOLÓGICA"].values
y_pred_demirjian = df_filtered["IDADE DENTÁRIA"].values

# Cálculo das métricas
mae_demirjian = mean_absolute_error(y_true, y_pred_demirjian)
rmse_demirjian = np.sqrt(mean_squared_error(y_true, y_pred_demirjian))
r2_demirjian = r2_score(y_true, y_pred_demirjian)

# Bootstrap para IC95%
n_iterations = 1000
np.random.seed(42)  # Para consistência
mae_bootstrap, rmse_bootstrap, r2_bootstrap = [], [], []
for _ in range(n_iterations):
    y_true_sample, y_pred_sample = resample(y_true, y_pred_demirjian)
    mae_bootstrap.append(mean_absolute_error(y_true_sample, y_pred_sample))
    rmse_bootstrap.append(np.sqrt(mean_squared_error(y_true_sample, y_pred_sample)))
    r2_bootstrap.append(r2_score(y_true_sample, y_pred_sample))

# IC 95%
mae_ci95 = np.percentile(mae_bootstrap, [2.5, 97.5])
rmse_ci95 = np.percentile(rmse_bootstrap, [2.5, 97.5])
r2_ci95 = np.percentile(r2_bootstrap, [2.5, 97.5])

# Exibir resultados
results_df = pd.DataFrame({
    "Métrica": ["MAE", "RMSE", "R²"],
    "Valor Estimado": [mae_demirjian, rmse_demirjian, r2_demirjian],
    "IC 95% (Inferior)": [mae_ci95[0], rmse_ci95[0], r2_ci95[0]],
    "IC 95% (Superior)": [mae_ci95[1], rmse_ci95[1], r2_ci95[1]]
})
print(results_df)

import matplotlib.pyplot as plt

# Criando a figura com seis subplots (5 modelos + 1 para o método Demirjian)
fig, axs = plt.subplots(2, 3, figsize=(18, 12))

# Model names for labeling
model_names = ['Linear Regression', 'Gradient Boosting', 'SVR', 'KNeighbors', 'Random Forest']

# Model predictions with correct variable names
model_predictions = [
    y_pred_test_linear2,  # Linear Regression
    y_pred_test_gb2,      # Gradient Boosting
    y_pred_test_svm2,     # SVR
    y_pred_test_knn2,     # KNeighbors
    y_pred_test_rf2       # Random Forest
]

# Metrics and confidence intervals for each model
metrics = [
    (mse_linear, rmse_linear, mae_linear, r2_linear, mae_ci95_linear, rmse_ci95_linear, r2_ci95_linear),  # Linear Regression
    (mse_gb, rmse_gb, mae_gb, r2_gb, mae_ci95_gb, rmse_ci95_gb, r2_ci95_gb),                              # Gradient Boosting
    (mse_svm, rmse_svm, mae_svm, r2_svm, mae_ci95_svm, rmse_ci95_svm, r2_ci95_svm),                      # SVR
    (mse_knn, rmse_knn, mae_knn, r2_knn, mae_ci95_knn, rmse_ci95_knn, r2_ci95_knn),                      # KNeighbors
    (mse_rf, rmse_rf, mae_rf, r2_rf, mae_ci95_rf, rmse_ci95_rf, r2_ci95_rf)                              # Random Forest
]

# Plotting for each model
for i, (model_name, y_pred, metric) in enumerate(zip(model_names, model_predictions, metrics)):
    row, col = divmod(i, 3)
    axs[row, col].scatter(y_test, y_pred, color='blue', label='Predicted values')
    axs[row, col].plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], color='red', linewidth=2, label='Identity line')
    axs[row, col].set_title(f'{model_name}: Actual vs Predicted')
    axs[row, col].set_xlabel('Actual Values')
    axs[row, col].set_ylabel('Predicted Values')
    axs[row, col].legend()
    axs[row, col].grid(True)

    # Adding metrics and confidence intervals to the plot
    mse, rmse, mae, r2, mae_ci, rmse_ci, r2_ci = metric
    axs[row, col].text(
        0.95, 0.05,
        f'MSE: {mse:.2f}\nRMSE: {rmse:.2f} ({rmse_ci[0]:.2f}, {rmse_ci[1]:.2f})\n'
        f'MAE: {mae:.2f} ({mae_ci[0]:.2f}, {mae_ci[1]:.2f})\nR2: {r2:.2f} ({r2_ci[0]:.2f}, {r2_ci[1]:.2f})',
        transform=axs[row, col].transAxes, fontsize=10, verticalalignment='bottom', horizontalalignment='right',
        bbox=dict(boxstyle='round', facecolor='white', alpha=0.5)
    )

# Scatter plot for 'Demirjian Method' (now with Actual on X and Predicted on Y)
axs[1, 2].scatter(y_test, X_test2['IDADE DENTÁRIA'], color='purple', label='Predicted values')
axs[1, 2].plot([y_test.min(), y_test.max()], [X_test2['IDADE DENTÁRIA'].min(), X_test2['IDADE DENTÁRIA'].max()], color='red', linewidth=2, label='Identity line')
axs[1, 2].set_title('Demirjian Method: Actual vs Predicted')
axs[1, 2].set_xlabel('Actual Values')
axs[1, 2].set_ylabel('Predicted Value by Demirjian Method')
axs[1, 2].legend()
axs[1, 2].grid(True)

# Add metrics and confidence intervals for the Demirjian Method
axs[1, 2].text(
    0.95, 0.05,
    f'MSE: {mse_dental_age:.2f}\nRMSE: {rmse_dental_age:.2f} ({rmse_ci95[0]:.2f}, {rmse_ci95[1]:.2f})\n'
    f'MAE: {mae_dental_age:.2f} ({mae_ci95[0]:.2f}, {mae_ci95[1]:.2f})\nR2: {r2_dental_age:.2f} ({r2_ci95[0]:.2f}, {r2_ci95[1]:.2f})',
    transform=axs[1, 2].transAxes, fontsize=10, verticalalignment='bottom', horizontalalignment='right',
    bbox=dict(boxstyle='round', facecolor='white', alpha=0.5)
)
...
# Function to calculate metrics and confidence intervals with bootstrap
...
# Models, their predictions, and titles
models = {
    ...
    "MLP": y_pred_test_mlp2,
    "Decision Tree": y_pred_test_dt2,
    "AdaBoost": y_pred_test_ada2,
    "Demirjian Method": X_test2["IDADE DENTÁRIA"]
}
...
for i, (model_name, y_pred) in enumerate(models.items()):
    metrics = calculate_metrics_with_ci(np.array(y_test), np.array(y_pred))
    mae, mae_ci = metrics["MAE"]
    rmse, rmse_ci = metrics["RMSE"]
    r2, r2_ci = metrics["R²"]

    # Check for Demirjian Method and change the color to purple
    color = 'purple' if model_name == "Demirjian Method" else 'blue'

    # Scatter plot
    axs[i].scatter(y_test, y_pred, color=color, alpha=0.6, label='Predicted values')
    axs[i].plot([y_test.min(), y_test.max()], [y_test.min(), y_test.max()], color='red', linestyle='--', label='Identity line')
    axs[i].set_title(f"{model_name}: Actual vs Predicted", fontsize=12)
    axs[i].set_xlabel("Actual Values", fontsize=10)
...
# Remove unused subplot if there are any
...
# Criar o gráfico corrigido com a legenda dentro do quadro do gráfico
plt.figure(figsize=(16, 10))
bars = plt.bar(
    model_names, mean_differences,
    yerr=[np.abs(np.array(mean_differences) - np.array(lower_bounds)),
          np.abs(np.array(upper_bounds) - np.array(mean_differences))],
    capsize=6, color='lightcoral', edgecolor='black', alpha=0.9, width=0.5
)
...
for i, bar in enumerate(bars):
    ...
    y_pos = +0.1  # Ajustar a posição relativa
    text = f"p = {p_values[i]:.4f}\nΔ = {mean_differences[i]:.2f}"
    plt.text(x_pos, y_pos, text, ha='center', va='center', fontsize=10,
             bbox=dict(facecolor='white', alpha=0.8))
...
plt.xticks(rotation=45, ha='right', fontsize=12)
plt.yticks(fontsize=12)
plt.xlabel('Model Comparisons', fontsize=14, labelpad=10)
plt.ylabel('Difference in MAE', fontsize=14, labelpad=10)
plt.title('Comparison of MAE between Models and Demirjian Method (95% CI)', fontsize=16, weight='bold')
plt.grid(axis='y', linestyle='--', alpha=0.6)

# Legenda principal
legend_labels = ['MAE Higher than Demirjian', 'MAE Lower than Demirjian']
legend_handles = [plt.Line2D([0], [0], color='lightblue', lw=10),
                  plt.Line2D([0], [0], color='lightcoral', lw=10)]
plt.legend(legend_handles, legend_labels, loc='upper left', fontsize=12, frameon=True, facecolor='white', edgecolor='black')

# Adicionar legenda sutil dentro do quadro do gráfico, no canto inferior direito
plt.text(plt.xlim()[1] - 0.1, plt.ylim()[0] + 1.515,
...
n = len(y_pred_dmj)  # Número de observações
z_value = 1.96       # Valor crítico para IC 95%
...
for model_name, y_pred in y_preds.items():
    # Diferença entre previsões
    differences = y_pred - y_pred_dmj
    ...
    # IC 95%
    lower_bound = mean_diff - z_value * se_diff
    upper_bound = mean_diff + z_value * se_diff
    lower_bounds.append(lower_bound)
    upper_bounds.append(upper_bound)

    # Teste t pareado
    t_stat, p_value = ttest_rel(y_pred, y_pred_dmj)
...
for i, model_name in enumerate(model_names):
    ...
    print(f"Mean Difference: {mean_differences[i]:.4f}")
    print(f"95% CI: ({lower_bounds[i]:.4f}, {upper_bounds[i]:.4f})")
    print(f"p-value: {p_values[i]:.4f}")
    print(f"Cohen's d: {cohen_ds[i]:.4f}")
    print("-" * 40)

# Atualizar o valor correto de MAE do método de Demirjian
mae_demirjian_mean = 1.3385582753200091

# Atualizar o teste t pareado com o MAE correto e calcular o d de Cohen
t_test_results = {}

for model, mae_values in metrics_test.items():
    # Teste t pareado
    t_stat, p_value = stats.ttest_rel(mae_values, [mae_demirjian_mean] * len(mae_values))

    # Calcular diferença média
    mean_diff = np.mean(np.array(mae_values) - mae_demirjian_mean)

    # Calcular o desvio padrão das diferenças para o cálculo do d de Cohen
    std_diff = np.std(np.array(mae_values) - mae_demirjian_mean, ddof=1)

    # Calcular Cohen's d
    cohen_d = mean_diff / std_diff if std_diff != 0 else 0

    # Armazenar resultados
    t_test_results[model] = {
        "MAE Model": np.mean(mae_values),
        "MAE Demirjian": mae_demirjian_mean,
...
df_results = pd.DataFrame.from_dict(t_test_results, orient='index')
